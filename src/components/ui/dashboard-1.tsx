...
export const MarketingDashboard = React.forwardRef<HTMLDivElement, MarketingDashboardProps>(({ 
  title = "Marketing Activities", teamActivities, team, cta, onFilterClick, className 
}, ref) => {
  ...
  return (
    <motion.div ref={ref} className={cn("w-full max-w-2xl p-6 bg-card text-card-foreground rounded-2xl border dark:border-white/10 dark:bg-zinc-950", className)} variants={containerVariants} initial="hidden" animate="visible">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <motion.div variants={itemVariants} whileHover={{ scale: 1.03, y: -5 }} transition={hoverTransition}>
          <Card className="h-full p-4 overflow-hidden rounded-xl border-outline-variant/30 dark:bg-zinc-900/50">
            <CardContent className="p-2">
              ...
              <div className="mb-4">
                <span className="text-4xl font-bold"><AnimatedNumber value={teamActivities.totalHours} /></span>
              ...
              <div className="w-full h-2 mb-2 overflow-hidden rounded-full bg-muted flex">
                {teamActivities.stats.map((stat, index) => (
                  <motion.div key={index} className={cn("h-full", stat.color)} initial={{ width: 0 }} animate={{ width: `${stat.value}%` }} transition={{ duration: 1, delay: 0.5 + index * 0.1 }} />
                ))}
              </div>
        ...
        <motion.div variants={itemVariants} whileHover={{ scale: 1.03, y: -5 }} transition={hoverTransition}>
          <Card className="h-full p-4 overflow-hidden rounded-xl bg-lime-50 dark:bg-lime-900/10 border-lime-200 dark:border-lime-900/30">
            <CardContent className="p-2">
              <div className="flex items-center justify-between mb-4">
              ...
              <div className="mb-6">
                <span className="text-4xl font-bold text-lime-950 dark:text-lime-50"><AnimatedNumber value={team.memberCount} /></span>
                <span className="ml-1 text-lime-800 dark:text-lime-300">members</span>
              </div>
              <div className="flex -space-x-2">
                {team.members.slice(0, 4).map((member, index) => (
                  <motion.div key={member.id} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.8 + index * 0.1 }} whileHover={{ scale: 1.2, zIndex: 10, y: -2 }}>
      ...
      <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }} transition={hoverTransition} className="mt-4">
         <div className="flex items-center justify-between p-4 rounded-xl bg-muted/60 dark:bg-zinc-900/50">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-full bg-background dark:bg-zinc-800"><Zap className="w-5 h-5 text-foreground" /></div>
              <p className="text-sm font-medium text-muted-foreground">{cta.text}</p>
            </div>
            <Button onClick={cta.onButtonClick} className="shrink-0 bg-primary text-on-primary">
                {cta.buttonText}
